...
Set of groups of metrics to compute. If not specified, re_data will compute metrics defined by `re_data:default_metrics` variable. See below for more details.
...
Groups of metrics to compute. By defult `table_metrics` and `column_metrics` are defined here, but you 
can redefine them or add a new groups. Those groups can be later passed to specific model configurations as well as
passed to `re_data:default_metrics` variable.

### re_data:default_metrics

Default metrics to compute for each model if no `re_data_metrics` or `re_data_metrics_groups` are specified.
...
Variable to enable storing test history. See [re_data tests history](/docs/reference/tests/history) for more details.
...
Variable to configure owners for your data.  See [re_data notifications](/docs/reference/notifications/configuring_owners) for more details.