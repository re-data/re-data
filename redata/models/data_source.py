...
class DataSource(Base):
    ...
    def get_db_object(self):
        ...
        if self.source_type == 'exa+pyexasol':
            return Exasol(self.name, ExasolEngine(db_url), schema=self.schema)

        if self.source_type == 'bigquery':
            db = create_engine(db_url, credentials_path=settings.REDATA_BIGQUERY_DOCKER_CREDS_FILE_PATH)
            return BigQuery(self.name, db, schema=self.schemas)
        
        db = create_engine(db_url)

        if self.source_type == 'postgres':
            return Postgres(self.name, db, schema=self.schemas)

        if self.source_type == 'mysql':
            return MySQL(self.name, db, schema=self.schemas)
    ...
    def source_dbs(cls):
        if not getattr(cls, 'source_db_objects', None):
            sources = metrics_session.query(cls).filter(cls.active == True).all()
